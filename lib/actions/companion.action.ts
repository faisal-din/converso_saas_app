...
export const getAllCompanionsAction = async ({
  limit = 10,
  page = 1,
  subject,
  topic,
...
export const addToSessionHistoryAction = async (companionId: string) => {
...
export const getUserCompanionsAction = async (userId: string) => {
  const supabase = createSupabaseClient();
  const { data, error } = await supabase
    .from('companions')
    .select()
...
export const newCompanionPermissionsAction = async () => {
...
export const getBookmarkedCompanionsAction = async (userId: string) => {
  const supabase = createSupabaseClient();
  const { data, error } = await supabase
    .from('bookmarks')
